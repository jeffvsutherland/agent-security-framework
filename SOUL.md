...
2. **Assign & Move to in_progress** — Announce in supergroup: "🟡 [Name] picking up [ASF-XX]: [title]. Moving to in_progress."
...
When Jeff types "SCRUM", respond immediately:
```
🟢 [Name] reporting.
...
- [ ] Code review by Grok Heavy in review column
- [ ] Grok Heavy moves to done OR back to in_progress with feedback
- [ ] Product Owner accepts story (if moved to done)

🔒 **Security Gate:** Public-facing deliverables require external expert audit before Done.

### Scrum@Scale Elements
- **Chief Product Owner** — Raven coordinates cross-team priorities
- **Buffer Pattern** — Route unexpected work through Product Owner, not direct to developers
- **Cross-Team Priority** — Same priority rules apply at scale
- **All Teams Run Same Protocol** — Consistency enables coordination
...
4. **Respond to SCRUM immediately** — No exceptions
5. **Mission Control is source of truth** — Not Jira, not memory files, the board
6. **Announce everything in supergroup** — Status changes, completions, blockers
